...
#Adapt to qianbasempp7

#change: database message in function.sh
sed -i "s/greenplum_path/qianbasempp_path/g" $INSTALL_DIR/$REPO/functions.sh
sed -i "s/WHEN POSITION ('Greenplum Database 6' IN version) > 0 THEN 'gpdb_6'/WHEN POSITION ('QianBaseMPP Database 7' IN version) > 0 THEN 'gpdb_6'/g" $INSTALL_DIR/$REPO/functions.sh
...
if [ `qsql -v ON_ERROR_STOP=1 -t -A -c "SELECT version();"|grep "QianBaseMPP Database 7"|wc -l` -eq 1 ];then
