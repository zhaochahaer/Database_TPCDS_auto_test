...
sed -i "s/gpstate/qbstate/g" $INSTALL_DIR/$REPO/01_gen_data/rollout.sh
...
sed -i "s/gpconfig/qbconfig/g" $INSTALL_DIR/$REPO/02_init/rollout.sh
sed -i "s/gpstop/qbstop/g" $INSTALL_DIR/$REPO/02_init/rollout.sh
sed -i "s/postgresql.conf/qianbase.conf/g" $INSTALL_DIR/$REPO/02_init/rollout.sh
...
if [ `psql -v ON_ERROR_STOP=1 -t -A -c "SELECT version();"|grep "QianBaseMPP Database 6"|wc -l` -eq 1 ];then
