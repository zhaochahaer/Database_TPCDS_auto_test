...
gen_data()
{
	...
	if [[ "$VERSION" == *"gpdb"* ]]; then
		PARALLEL=$(qstate | grep "Total primary segments" | awk -F '=' '{print $2}')
		if [ "$PARALLEL" == "" ]; then
			echo "ERROR: Unable to determine how many primary segments are in the cluster using qstate."
			exit 1
		fi
		echo "parallel: $PARALLEL"
		if [ "$VERSION" == "gpdb_6" ]; then
			for i in $(qsql -v ON_ERROR_STOP=1 -q -A -t -c "select row_number() over(), g.hostname, g.datadir from gp_segment_configuration g where g.content >= 0 and g.role = 'p' order by 1, 2, 3"); do
			...
			for i in $(qsql -v ON_ERROR_STOP=1 -q -A -t -c "select row_number() over(), g.hostname, p.fselocation as path from gp_segment_configuration g join pg_filespace_entry p on g.dbid = p.fsedbid join pg_tablespace t on t.spcfsoid = p.fsefsoid where g.content >= 0 and g.role = 'p' and t.spcname = 'pg_default' order by 1, 2, 3"); do
				CHILD=$(echo $i | awk -F '|' '{print $1}')
				EXT_HOST=$(echo $i | awk -F '|' '{print $2}')
				GEN_DATA_PATH=$(echo $i | awk -F '|' '{print $3}')
				GEN_DATA_PATH="$GEN_DATA_PATH""/pivotalguru"
