...
get_qbfdist_port()
{
	all_ports=$(qsql -t -A -c "select min(case when role = 'p' then port else 999999 end), min(case when role = 'm' then port else 999999 end) from gp_segment_configuration where content >= 0")
...
source_bashrc()
{
	...
	count=$(grep -v "^#" ~/.bashrc | grep "qianbasempp_path" | wc -l)
	if [ "$count" -eq "0" ]; then
		get_version
		if [[ "$VERSION" == *"gpdb"* ]]; then
			echo "$startup_file does not contain qianbasempp_path.sh"
...
get_version()
{
	...
	VERSION=$(qsql -v ON_ERROR_STOP=1 -t -A -c "SELECT CASE WHEN POSITION ('Greenplum Database 4.3' IN version) > 0 THEN 'gpdb_4_3' WHEN POSITION ('Greenplum Database 5' IN version) > 0 THEN 'gpdb_5' WHEN POSITION ('QianBaseMPP Database 7' IN version) > 0 THEN 'gpdb_6' ELSE 'postgresql' END FROM version();") 
	if [[ "$VERSION" == *"gpdb"* ]]; then
		quicklz_test=$(qsql -v ON_ERROR_STOP=1 -t -A -c "SELECT COUNT(*) FROM qb_compression WHERE compname = 'quicklz'")
...
create_hosts_file()
{
	...
	if [[ "$VERSION" == *"gpdb"* ]]; then
		qsql -v ON_ERROR_STOP=1 -t -A -c "SELECT DISTINCT hostname FROM gp_segment_configuration WHERE role = 'p' AND content >= 0" -o $LOCAL_PWD/segment_hosts.txt
	else
		#must be PostgreSQL
