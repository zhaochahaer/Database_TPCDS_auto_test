...
sed -i "s/WHEN POSITION ('Greenplum Database 6' IN version) > 0 THEN 'gpdb_6'/WHEN POSITION ('Greenplum Database 7' IN version) > 0 THEN 'gpdb_6'/g" $INSTALL_DIR/$REPO/functions.sh
...
if [ `psql -v ON_ERROR_STOP=1 -t -A -c "SELECT version();"|grep "Greenplum Database 7"|wc -l` -eq 1 ];then
